...
from constants import IS_MULTILINGUAL
if IS_MULTILINGUAL:
    from transformers.models.mt5.modeling_mt5 import MT5_INPUTS_DOCSTRING as T5_INPUTS_DOCSTRING
    from transformers.models.mt5.modeling_mt5 import __HEAD_MASK_WARNING_MSG as \
...
_TOKENIZER_FOR_DOC = "MT5Tokenizer" if IS_MULTILINGUAL else "T5Tokenizer"
_CHECKPOINT_FOR_DOC = "mt5-small" if IS_MULTILINGUAL else "t5-small"
...
class ConstrainedT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config: T5Config, special_ids: Dict,
        ...
        special_starts = [v for k,v in special_ids.items() if k.endswith("_start") and not k.startswith("sentence")]
        special_ends = [v for k,v in special_ids.items() if k.endswith("_end") and not k.startswith("sentence")]
    ...
    def input_to_actions(self, input_ids: torch.LongTensor):
        ...
        if self.action_type == 'integer':
            ...
            is_end = torch.isin(input_ids, self.special_ends)  # (input_ids == self.mention_end)
            is_start = torch.isin(input_ids, self.special_starts)  # (input_ids == self.mention_start)
            ...
            is_not_start = torch.isin(input_ids, self.special_starts, invert=True)  # (input_ids != self.mention_start)
            if self.add_mention_end:
                is_not_end = torch.isin(input_ids, self.special_ends, invert=True)  # (input_ids != self.mention_end)
