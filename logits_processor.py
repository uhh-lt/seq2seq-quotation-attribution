...
class IntProcessor(LogitsProcessor):
    ...
    def __init__(self, orig_inputs, special_ids, seq2seq_type):
        ...
        if (
            self.seq2seq_type == "action"
            or self.seq2seq_type == "tagging"
            or self.seq2seq_type == "input_feed"
        ):
    ...
    def __call__(
        ...
        is_sep = input_ids == self.sep
        is_end = torch.isin(
            input_ids, self.special_ends
        )  # (input_ids == self.mention_end)
        is_start = torch.isin(
            input_ids, self.special_starts
        )  # (input_ids == self.mention_start)
        is_ent = (is_sep.cumsum(-1) - is_end.cumsum(-1)).bool()
        ...
        if self.seq2seq_type == "tagging":
            ...
            if self.seq2seq_type == "action" or self.seq2seq_type == "input_feed":
...
class NonIntProcessor(LogitsProcessor):
    ...
    def __init__(self, orig_inputs, special_ids, seq2seq_type, add_mention_end):
        """
        ...
        self.cluster_ids = torch.tensor(special_ids["cluster_ids"], dtype=torch.long)
    ...
    def __call__(
        ...
        cluster_input_ids = input_ids.masked_fill(is_not_cid, self.first_cluster_id - 1)
        ...
        if self.add_mention_end:
            ...
            has_prev_ends = unclosed_ment & (next_cids > self.first_cluster_id)
        ...
        if self.add_mention_end:
            ...
            scores[unclosed_ment, next_cids[unclosed_ment]] = scores[
                unclosed_ment, self.cluster_new
            ]
