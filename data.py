...
class JointDataset(Dataset):
    ...
    def __init__(self, tokenizer, data_args, train_args, split):
        ...
        self.samples = (
            None
            if self.split == "train"
            else [s for data_samples in self.all_samples.values() for s in data_samples]
        )
    ...
    def _load_single_data(self, data_dir, data_name, max_len, thred):

        samples = []
        doc_labels = {}
        id_to_name = {}
        data_path = os.path.join(
            data_dir, f"{self.split}.t5-small.english.{max_len}.jsonlines"
        )
        with open(data_path, "r") as f:
            for line in f:
                ...
                if self.train_args.action_type == "integer":
                    ...
                    raise ValueError(
                        f"wrong action type " f"{self.train_args.action_type}"
                    )

                if (
                    self.train_args.seq2seq_type == "action"
                    or self.train_args.seq2seq_type == "input_feed"
                ):
                    if self.train_args.action_type == "integer":
                        ...
                        raise ValueError("wrong action type (" "integer/non_integer)")
                ...
                sample = {
                    ...
                    "seg_clusters": [
                        [tuple(m) for m in c]
                        for c in item["seg_clusters"]
                        if len(c) >= thred
                    ],
                    "offset": item["offset"],
                }
                doc_labels[doc_id] = [
                    [tuple(m) for m in c] for c in item["gold_clusters"]
                ]
    ...
    def load_dataset(self):
        ...
        samples = {}
        max_lens = (
            self.data_args.joint_max_train_lens.split(",")
            if self.split == "train"
            else self.data_args.joint_max_eval_lens.split(",")
        )
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            label_ids = torch.tensor(sample["target_sentence"], dtype=torch.long)
...
class CorefDataset(Dataset):
    ...
    def load_dataset(self):
        max_len = (
            self.data_args.max_train_len
            if self.split == "train"
            else self.data_args.max_eval_len
        )
        data_path = os.path.join(
...
def get_document_predicts(doc_preds: List[List]) -> List[List[Tuple[int, int]]]:
    """
    ...
    for component in nx.connected_components(graph):
        ...
        for start, end in sorted(component, key=lambda x: (x[0], -x[1])):
            ...
            condition = not any([s < start < e < end for (s, e) in processed_group])
...
def parse_int_output_tokens(
    input_ids, output_ids, special_ids, subtoken_map, tokenizer, thred, is_tagging
):
    special_starts = [
        v
    ...
    if is_tagging:
        new_input_ids = [
            special_ids["copy"]
            for t in input_ids
            if t != tokenizer.pad_token_id and t != special_ids["eos"]
        ]
    ...
    print(f"wrong matches: {wrong_matches} - correct matches: {correct_matches}")
    ...
    if wrong_reconstruction:
        ...
        predict_clusters = [
            list(set(v)) for k, v in clusters.items() if len(set(v)) >= thred
        ]
        ...
        predict_clusters = [list(set(v)) for v in clusters if len(set(v)) >= thred]
        token_mentions = [
...
def parse_short_target_tokens(
    ...
    # TODO: only match tags ofthe same type
...
def parse_nonint_output_tokens(
    input_ids, output_ids, special_ids, subtoken_map, tokenizer, add_mention_end, thred
):
    rec_ids, new_id = [], -1
    ment_start_stack = []
    unmatched_clusters = defaultdict(list)
    ...
    if wrong_reconstruction:
        ...
        print(f"reconstructed ids {rec_ids}")
        print(f"out ids {output_ids}")
        print("wrong reconstruction! please debug parse_nonint_output_tokens")
        matching = global_align(new_input_ids, rec_ids)
        ...
        clusters = [
            [(subtoken_map[m[0]], subtoken_map[m[1]]) for m in v]
            for v in unmatched_clusters.values()
        ]
        predict_clusters = [list(set(v)) for v in clusters if len(set(v)) >= thred]
        token_mentions = [(m[0], m[1]) for v in unmatched_clusters.values() for m in v]
        token_mentions = list(set(token_mentions))
